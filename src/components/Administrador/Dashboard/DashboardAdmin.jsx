...
export const DashboardAdmin = () => {
    ...
    const resultsClientes = clientes.filter((dato) => {
      if (selectClientes && selectClientes !== "" && dato[selectClientes]) {
        return dato[selectClientes].toLowerCase().includes(searchclientes.toLowerCase());
      }
      return true;
    });
    ...
    const resultsPrestamos = prestamos.filter((dato) => {
      if (selectPrestamos && selectPrestamos !== "" && dato[selectPrestamos]) {
        return dato[selectPrestamos].toLowerCase().includes(searchprestamos.toLowerCase());
      }
      return true;
    });
      ...
      <div className="adminContent-home">
        ...
        <div className="AdminContent-main">
          ...
          <select name="filtroPrestamo" onChange={valorSelectPrestamos}>
            ...
            />
                <button className="btn btn-primary"><FontAwesomeIcon className="icon" icon={faMagnifyingGlass} /></button>
            </div>
            <hr />
              ...
              <tbody>
                {resultsPrestamos.map((item, index) => (
                  <tr>
                    ...
                    <td><button type="button" class="btn btn-secondary"><FontAwesomeIcon className="icon" icon={faMagnifyingGlass} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
